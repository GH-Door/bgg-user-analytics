...
| 가설 | 왜 세웠나 |
|---|---|
| **H1.** 반복 플레이한 유저는 안 한 유저보다 실제 활동 기간이 더 길 것이다 | EDA에서 "같은 게임을 두 번 이상 했다"는 것까지만 확인, 그게 언제였는지 몰라 오래가는 참여인지 검증 필요 |
| **H2.** 보유 게임 수·활용도로 나눈 유형 간 활동 지속성 차이는 시간이 지나도 유지될 것이다 | 네 유형의 경계가 군집화로 어느 정도 뒷받침됐지만 전부 한 시점 데이터라, 시간이 지나도 유지되는지 불명 |
| **H3.** 최근에 가입한 유저일수록 실제로 게임을 플레이하는 비율이 더 낮을 것이다 | 가입자 수 흐름에 정체·감소 구간은 확인했지만, "가입 자체가 준 것"인지 "가입은 했는데 플레이로 안 이어진 것"인지 구분 안 됨 |

검증은 플레이 로그를 수집한 1,065명 중 실제 기록이 있는 730명을 대상으로 [04.Trend](jupyter/04.Trend.ipynb)에서 진행.

**가설 검증**

| 가설 | 검정 결과 | 판정 |
|---|---|:---:|
| H1 | 반복 플레이 있음 647명은 활동 연수 중앙값 8년·2026년까지 활동, 없음 82명은 1년·마지막 활동 7년 전(효과 크기 0.81, 매우 큼) | 채택 |
| H2 | 몰입형·수집가형·집중형은 지금도 활동 중, 저관여형만 마지막 활동 4~5년 전 — "위험군은 수집가형"이라는 사전 예상과 반대로 확인 | 부분 채택 |
| H3 | 기간 미통제 시 85.5%→47.0%로 하락처럼 보이나, 가입 후 1년 이내 기준으로 공정 비교하면 코호트 간 43~48%로 차이 거의 없음(카이제곱 p=0.635) — 1차 결과는 관측 기간 편향으로 판단 | 기각 |
...
| 세그먼트 | 인원 | 어떤 사람인가 |
|---|---:|---|
| 미플레이 | 956명 | 게임은 갖고 있지만 플레이한 적이 하나도 없음(평가 참여율 중앙값 82% — numplays가 자기입력값이라 검증 가능 표본 기준 최소 8.2%는 기록만 안 한 것으로 추정, [04.Trend 2-2](jupyter/04.Trend.ipynb)) |
...
이 경계는 실루엣 계수 0.32~0.35로 낮아 "데이터가 찾은 자연 군집"은 아니고, 분석 목적에 맞춰 정의한 틀임을 명시. 위시리스트에 담은 게임의 99%는 실제 플레이로 안 이어져, 위시리스트는 구매 예고가 아니라 관심 표시에 가까움.
...
|---|---|---|---|
| 최근 가입자(2021년 이후) | 가입 후 1년 이내 기록 보유율이 전 코호트 43~48%로 고르게 낮음 | 가입 후 3일 안에 유저가 실제로 한 행동에 맞춰 자동으로 안내 메일 발송: 가입 즉시 "첫 게임 등록하는 법" 안내 → 24시간 지나도 안 하면 리마인드 → 3일째까지 기록이 없으면 입문용 게임 추천. 정해진 시간마다 일괄 발송하는 것보다 유저 행동에 반응해 보내는 메일의 전환율이 2~3배 높고, 가입 후 3일 안에 아무 활동이 없으면 이탈 확률이 90% 뛴다는 SaaS 업계 사례 참고 | 가입 후 3일 내 첫 플레이 기록률(코호트별 비교) |
...
> - 각 단계는 체크포인트 기반이라 중단 후 재실행해도 이어서 진행.
