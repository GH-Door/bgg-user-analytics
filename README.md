...
> - 보드게임을 구매·보유해도 실제 플레이로 이어지지 않는 ["백로그"](https://tabletopstrategy.wordpress.com/2021/02/19/the-shelf-of-shame/) 현상이 커뮤니티에서 널리 알려져 있는데,
> - BGG는 보유 목록과 플레이 기록을 별도 필드로 관리해 이 간극을 데이터로 측정할 수 있다.
> - 다만 이 간극이 실제로 얼마나 크고 누구에게 더 크게 나타나는지는 정량적으로 검증된 적이 없어,
> - 3,000명의 보유·플레이 데이터를 직접 수집해, 보유가 플레이로 이어지지 않는 지점과 그 지점의 유저 특성을 분석했다.
...
- 퍼널 최대 손실 구간을 **보유 → 플레이함(-32.5%p, 956명)** 으로 특정
- 보유 게임 수만으로 유저를 나누면 안 된다는 것을 규명하고, **보유 게임 수와 활용도 두 축을 교차**해 4개 유형(미플레이 별도)으로 재정의
- 가설 3개 검정:
    - H1 채택(효과 크기 **0.81**)
    - H2 부분 채택
...
**H1.** 반복 플레이한 유저는 안 한 유저보다 활동 기간이 더 길 것이다.  
**이유**:
   - EDA에서 "같은 게임을 두 번 이상 했다"는 것까지만 확인
   - 그게 언제였는지 몰라 오래가는 참여인지 검증 필요

**H2.** 보유 게임 수 × 활용도 유형 간 활동 지속성 차이는 시간이 지나도 유지될 것이다.  
**이유**:
   - 4 유형의 경계가 군집화로 어느 정도 뒷받침됐지만
   - 전부 한 시점 데이터라, 시간이 지나도 유지되는지 불명

**H3.** 최근에 가입한 유저일수록 게임을 플레이하는 비율이 더 낮을 것이다.  
**이유**:
   - 가입자 수 정체·감소 구간은 확인했지만
   - "가입 자체가 준 것"인지 "가입은 했는데 플레이로 안 이어진 것"인지 구분 안 됨
...
| 세그먼트 | 인원 | 어떤 사람인가 |
|---|---:|---|
| 미플레이 | 956명 | 게임은 갖고 있지만 플레이 기록이 하나도 없음 ([04.Trend](jupyter/04.Trend.ipynb) 2-2) |
...
> 아래 액션은 위 Insight/Results에서 나온 제안.  
...
|:---:|---|---|---|---|
| 1 | 가입했거나 게임을 처음 등록한 직후 유저 | "첫 플레이 기록" 배지 주기<br>기록이 없으면 안내 메일 1통 | 가장 많이 빠지는 구간이 보유 → 플레이(-32.5%p, 956명)<br>가입 후 1년 안 기록 비율이 가입 시기 간 차이가 거의 없이 43~48%([04.Trend](jupyter/04.Trend.ipynb) 3-1) | 배지와 메일을 각각 따로, 본 그룹 vs 못 본 그룹 비교(A/B 테스트)<br>핵심: 첫 기록 남긴 비율<br>부작용: 메일 수신거부, 배지만 노린 가짜 기록 |
| 2 | 평점은 남겼지만 플레이 기록이 없는 유저 | 평점 남긴 직후 "플레이 기록도 남겨보세요" 안내 | 기록 없는 956명 중 99%가 평점은 남김([Segment 분석](jupyter/03.Segment_Wishlist.ipynb) 2-0)<br>최소 8.2%는 했는데 기록만 안 남긴 것으로 추정([04.Trend](jupyter/04.Trend.ipynb) 2-2) | 안내를 본 그룹 vs 못 본 그룹 비교<br>핵심: 안내 뒤 기록 남긴 비율<br>부작용: 평점 입력을 중간에 그만두는 비율 |
| 3 | 수집가형(많이 갖고 일부만 함, 지금도 활동 중) | 안 해본 보유 게임 중 자주 하는 게임과 비슷한 것(난이도·인원·시간)을 "다음에 할 게임"으로 추천 | 로그가 있는 수집가형 188명은 중앙값 기준 올해도 활동 중, 활동 기간 10년([04.Trend](jupyter/04.Trend.ipynb) 2절)<br>무엇을 할지 고르기가 걸림돌이라는 가설 | 먼저 과거 기록으로 추천이 맞았는지 확인, 이후 A/B 테스트<br>핵심: 추천 게임을 플레이한 비율<br>부작용: 추천 끄기·숨기기 비율 |
| 4 | 기록이 끊긴 유저(저관여형과 같은 패턴) | 마지막 기록 60일 뒤부터 메일 3통(안부, 취향 맞춤 소개, 마지막 안내)<br>이미 4~5년 떠난 유저는 소규모로 반응부터 확인 | 저관여형은 약 1년만 활동하고 4~5년째 떠나 있음([04.Trend](jupyter/04.Trend.ipynb) 2절)<br>2년 넘게 쉰 유저가 저절로 돌아올 확률은 1% 안팎, 60일 무렵은 10%대([04.Trend](jupyter/04.Trend.ipynb) 2-3) | 메일 받은 그룹 vs 안 받은 그룹 비교<br>핵심: 30일 안에 다시 기록 남긴 비율<br>부작용: 수신거부, 스팸 신고 |
| 5 | 집중형(후순위) | 위시리스트와 비슷한 난이도 추천 vs 지금 하는 게임과 비슷한 난이도 추천, 두 가지 비교 | 원하는 게임과 실제 하는 게임의 난이도 차이가 가장 큼(0.26), 이유는 알 수 없음([Segment 분석](jupyter/03.Segment_Wishlist.ipynb) 4-1) | 핵심: 추천 후 플레이 기록 남긴 비율<br>부작용: 게임 수만 늘고 플레이는 그대로인 경우 |
| 6 | 몰입형(후순위) | 보유 게임의 확장판·후속작 추천 | 몰입형만 선호 4~5위권에 확장판이 나타남, 가진 게임 계열을 반복해서 플레이([Segment 분석](jupyter/03.Segment_Wishlist.ipynb) 3-2) | 핵심: 추천으로 산 게임을 플레이한 비율<br>부작용: 게임 수만 늘고 플레이는 그대로인 경우 |

<details>
<summary><strong>4번 근거: 쉬는 기간이 길수록 돌아올 확률은 낮아짐</strong> (펼쳐보기)</summary>

<div align="center">
<img src="assets/return_prob.png" width="90%" alt="쉰 기간별 30일 내 복귀 확률과 마지막 접속 후 경과 기간 분포">
</div>

- 개입 없이 저절로 돌아오는 비율이라 메일 효과가 아니며, 실험에서 메일을 받지 않은 그룹의 기준값으로 사용([04.Trend](jupyter/04.Trend.ipynb) 2-3)
- 저관여형 대용(총 플레이 100회 이하 218명)은 60일 쉬면 30일 안에 돌아올 확률이 13.8%, 2년 쉬면 0.8%
- 2년 넘게 접속하지 않은 유저가 10.1%(302명)로 가장 큰 휴면 덩어리지만 복귀 확률이 1% 안팎이라, 이 구간은 메일 효율이 낮을 가능성
- 61~180일 쉬는 유저(213명)는 복귀 확률이 7~14% 남아 있어, 떠나는 중인 이 구간에 개입 여지가 더 큼
...
> - "가입 후 며칠", "등록 후 며칠" 등은 데이터로 알 수 없어(가입 연도만 제공, 게임 등록일 없음) 실험 전에 먼저 측정 필요
...
**Takeaways**
- HTTP 200 응답 안에 숨은 에러:
    - 존재하지 않는 계정으로 파일럿 테스트를 해보니, collection API는 무효 요청에도 HTTP 200을 주고 에러는 응답 본문에만 담았다.
    - 상태 코드만으로는 신뢰할 수 없다는 걸 체감하고 본문까지 파싱해 검사하도록 고쳤다.
- 회귀 변수의 데이터 누수:
    - 회귀에 넣으려던 `play_game_ratio`가 종속변수와 사실상 같은 값에서 나온 파생 지표라는 걸 발견했다.
    - 이런 지표를 걸러내지 않으면 회귀 계수·오즈비 해석 자체가 왜곡된다는 걸 깨달았다.
- 관측 기간을 안 맞춘 착시:
    - 가입연도별 플레이 비율을 단순 비교하면 85.5%→47.0%로 착시가 났다.
    - 가입 후 동일 기간으로 맞춰 비교하니 차이가 거의 사라져(43~48%, p=0.635) 관측 기간을 안 맞추면 착시가 생긴다는 걸 알게 됐다.

**Limitations**
- 접속 기록 부재:
    - 클릭스트림 같은 접속 기록이 없어서 퍼널과 코호트를 소유·플레이 여부로만 분석할 수밖에 없었다.
    - 세션 단위 행동이나 실제 활동 강도까지는 이 데이터로 볼 수 없어 아쉬웠다.
- 획득 이전 단계 데이터 부재:
    - 이 프로젝트의 퍼널은 게임을 보유한 이후만 다루는데, 
    - 관심을 갖고 위시리스트에 올렸다가 실제로 구매하기까지의 이전 단계는 **BGG API**에 데이터가 없다.
    - 그래서 이 구간은 퍼널에 포함하지 못해 아쉬웠다.
- 다중비교 미보정:
    - 여러 차례 검정을 하면서 다중비교 보정을 따로 하지 않았다.
    - 개별 p-value보다는 효과 크기와 방향성 위주로 해석해서 보완했다.
