...
|---|---|---|---|
| 게임을 산 직후의 모든 유저 | 등록 즉시 배지(완료 보상 아이콘) 지급<br>7일 미기록 시 플레이 안내 | 퍼널 최대 손실 구간(-32.5%p) | 배지 노출군 vs 비노출군<br>플레이 전환율 A/B 테스트 |
| 최근 가입자(2021년 이후) | 가입 후 3일 내 행동 기반<br>온보딩 메일 자동 발송 | 가입 후 1년 이내 기록 보유율<br>전 코호트 43~48%로 고르게 낮음 | 가입 후 3일 내<br>플레이 기록률(코호트별) 확인 |
...
| 몰입형(많이 갖고 잘 쓰는 유저) | 보유 게임의<br>확장판·후속작 우선 추천 | 선호 카테고리 4~5위권에<br>"확장판" 유독 높음([Segment 분석](jupyter/03.Segment_Wishlist.ipynb) 3-2) | 추천 클릭률<br>추천 후 구매·플레이 전환율 확인 |
